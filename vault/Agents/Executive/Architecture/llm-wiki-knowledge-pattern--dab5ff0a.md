...
Obsidience uses a Karpathy-style wiki as the durable brain of the agent fleet. Knowledge compounds in concise, linked, human-editable Articles instead of being buried in transcripts or one oversized prompt. Agents continuously ingest, improve, verify, and retrieve this wiki through explicit Tasks and Runbooks.

## Layers

1. **Source** preserves immutable external blobs such as captured pages, documents, code, data, and images.
...
The graph stays useful by changing accepted Articles in place, removing duplicates and filler, and preserving exact Source material outside retrieval. Similarity supplies context; exact graph edges select what may run.

## Maintenance loop

- Darwin researches one bounded gap and preserves direct evidence.
- Alexandria reconciles the finding into the smallest coherent set of Knowledge Articles.
