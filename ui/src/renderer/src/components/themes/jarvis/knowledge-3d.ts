...
// repulsion, hard collision on each node's avoidance radius, and a weak
// per-depth radial force that keeps the hierarchy radiating from the
// pinned Brain as a ball. Nodes seed OUTSIDE their neighborhoods on
// deterministic branch-cone directions and visibly fall/wobble into place
...
export const KNOWLEDGE_3D_TUNING_FIELDS: readonly Knowledge3dTuningField[] = [
  ...
  { key: "sweepSpeed", label: "Animation speed", group: "Thinking", min: 0.1, max: 3, step: 0.05, description: "Speed of the thinking animation along the lit path, in path-units per second." },
  ...
  { key: "subjectStyle", label: "Branch nodes", group: "Nodes", min: 0, max: 3, step: 1, options: ["Classic disc", "Plasma orb", "Gem", "Hollow"], description: "Disc style for top-level branch subjects." },
  { key: "subnodeStyle", label: "Sub-branch nodes", group: "Nodes", min: 0, max: 3, step: 1, options: ["Classic disc", "Plasma orb", "Gem", "Hollow"], description: "Disc style for deeper subject nodes." },
  { key: "articleStyle", label: "Article nodes", group: "Nodes", min: 0, max: 3, step: 1, options: ["Jewel star", "Plasma orb", "Classic disc", "Ember"], description: "Sprite style for the article orbs." },
  ...
  { key: "crossCurve", label: "Link curve", group: "Beams", min: 0, max: 0.6, step: 0.01, description: "How strongly article links bow outward from the ball's center." },
...
/** The ONE spacing constant (owner 2026-08-04, no-rings law): every
 *  parent-child spring rests at this length × the Link distance slider.
 *  The 90° dome + collision avoidance grow the ball from it naturally. */
...
/** Build the live simulation: springs + charge + collision + weak radial
 *  shells, Brain pinned at the origin, stopped (the scene owns the tick
 *  cadence). d3-force uses a deterministic internal LCG, so identical
...
export function createKnowledgeForceSimulation(
  ...
  return forceSimulation(nodes, dimensions)
    ...
    .force(
      "dagRadial",
      forceRadial((node: KnowledgeForceNode) =>
...
export function forceKnowledgeLeafHemisphere(
  ...
  // Every child with a non-root parent (owner 2026-08-04, no-rings law:
  // subjects AND leaves) — outward monotonicity replaces the shells.
...
export function knowledge3dTuningKey(agentId?: string): string {
  return agentId === undefined || agentId === "main"
    ? "herebrum.jarvis-knowledge.tuning3d.v3"
    : `herebrum.jarvis-knowledge.tuning3d.${agentId}.v2`;
}

/** The retired satellite v1 key — read once for the v2 migration. */
export function knowledge3dTuningKeyV1(agentId: string): string {
  return `herebrum.jarvis-knowledge.tuning3d.${agentId}.v1`;
}
...
export const KNOWLEDGE_2D_TUNING_FIELDS: readonly Knowledge3dTuningField[] = [
  ...
  { key: "crossCurve", label: "Link curve", group: "Article links", min: 0, max: 0.6, step: 0.01 },
...
/** Delivery comet timing (owner 2026-08-04: when a satellite's job files an
