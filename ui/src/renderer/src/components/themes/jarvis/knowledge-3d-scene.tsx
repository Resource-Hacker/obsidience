...
export interface Knowledge3dSceneProps {
  ...
  satelliteSweeps?: ReadonlyArray<{
    ...
    key: number;
    /** Job sweeps HOLD after the run (path stays lit, dashes streaming)
     *  until the backdrop clears them at job end; test sweeps clear on the
  ...
  /** Delivery comets (owner 2026-08-04): a finished job filed an article
...
void main() {
  ...
  if (vStyle.x > 0.5) {
    ...
    if (vStyle.x < 1.5) {
      ...
      ringA = vRing.a * (1.0 - smoothstep(halfW, halfW + aa, abs(r - vStyle.y)));
      ...
      if (uRingStyle > 2.5) {
        ...
        float inner = vRing.a *
          (1.0 - smoothstep(halfW, halfW + aa, abs(r - vStyle.y * 0.78)));
    ...
    if (vStyle.x > 1.5) {
      ...
      // the shared library's core is deliberately GEOMETRIC so Athenaeum
      ...
      // 0 Classic disc, 1 Plasma orb, 2 Gem, 3 Hollow.
      float variant = vStyle.x > 1.1 ? uSubnodeStyle : uSubjectStyle;
    ...
    if (uArticleStyle > 1.5 && uArticleStyle < 2.5) {
      float t = clamp((length(p - vec2(-0.28, -0.32)) - 0.08) / 0.92, 0.0, 1.0);
      vec3 flat3 = t < 0.28
        ? mix(vec3(0.941, 0.976, 1.0), vCore, t / 0.28)
        : mix(vCore, vDark, (t - 0.28) / 0.72);
...
export function Knowledge3dScene(props: Knowledge3dSceneProps) {
  ...
  useEffect(() => {
    ...
    // Per-agent satellite sweeps (job thinking runs on EVERY cloud with a
    // live job, concurrently): each cloud's clock starts when its entry's
    ...
    >();
    // Frame-gate memory: in-flight sweeps/comets earn 60 fps, a HELD job
    ...
    function frame(now: number): void {
      ...
      if (spec !== appliedPathSpec) {
        ...
        // rebase a plan that GROWS mid-flight (a later Hermes retrieval
      ...
      // on completion, then segmented flow. Job thinking (owner 2026-08-04)
      // runs one sweep per working agent CONCURRENTLY and HOLDS the lit
      // path (dashes streaming) until the job finishes; a pulseKey bump
