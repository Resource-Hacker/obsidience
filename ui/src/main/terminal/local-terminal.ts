/** Fixed local tmux mirror, adapted from HEREBRUM's accepted Local Console. */
...
export class LocalTerminalManager {
  ...
  private ensureLinkedSession(): void {
    ...
    // This is the same pane currently shown by the physical codex-dp4 client.
    this.command(TMUX_BINARY, ["select-window", "-t", `=${LOCAL_TERMINAL_SESSION}:JARVIS`]);
  }

  private inspectTmuxWindowSize(fallback: LocalTerminalSize): LocalTerminalSize {
    const inspected = this.command(TMUX_BINARY, [
      "display-message", "-p", "-t", `${LOCAL_TERMINAL_SESSION}:JARVIS`,
