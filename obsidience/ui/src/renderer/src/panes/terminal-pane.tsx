/** Terminal pane: Obsidience-style tmux mirror plus live Obsidience actions. */
...
const BASE_LINE_HEIGHT = 1.12;
const MAX_LINE_HEIGHT = 10;

function responsiveCellSpacing(
  ...
  if (
    ...
    || !Number.isFinite(measuredCells)
    || !Number.isFinite(targetCells)
    || !Number.isFinite(availablePixels)
    || measuredCells < 1
    || targetCells < 1
    || availablePixels <= 0
  ) return BASE_SPACING;

  const measuredCellWidth = availablePixels / measuredCells;
  const adjustment = measuredCellWidth * (measuredCells / targetCells - 1);
  return Math.min(MAX_SPACING, Math.max(BASE_SPACING, currentSpacing + adjustment));
}

function responsiveLineHeight(
  ...
  return Math.min(
    MAX_LINE_HEIGHT,
    Math.max(BASE_LINE_HEIGHT, currentLineHeight * (measuredRows / targetRows)),
  );
}
...
function LocalConsoleView() {
  ...
  useEffect(() => {
    ...
    const fit = () => {
      ...
      try {
        if (targetColumns === null) {
          ...
          // A short pane must still admit the complete tmux window and status
          // row. Reduce the font only when the normal line height would crop
          // that fixed view; horizontal spacing is resolved afterward.
          if (targetRows !== null) {
            for (let i = 0; i < 5; i += 1) {
              const proposed = fitAddon.proposeDimensions();
              if (!proposed || proposed.rows >= targetRows) break;
              const current = terminal.options.fontSize ?? BASE_FONT;
              const next = Math.max(MIN_FONT, current * (proposed.rows / targetRows));
              if (Math.abs(next - current) < 0.01) break;
              terminal.options.fontSize = next;
            }
          }

          // The smaller Obsidience-density font keeps more rows visible, while
          // bounded cell spacing stretches the fixed tmux grid back across
          // the complete pane width. Without this second pass the terminal
          // occupies only 80% of its host after every resize.
          for (let i = 0; i < 5; i += 1) {
            const proposed = fitAddon.proposeDimensions();
            if (!proposed || proposed.cols === targetColumns) break;
            const availableWidth = terminal.element?.clientWidth ?? host.clientWidth;
            const current = terminal.options.letterSpacing ?? BASE_SPACING;
            const next = responsiveCellSpacing(
              current,
              proposed.cols,
              targetColumns,
              availableWidth,
            ...
            if (Math.abs(next - current) < 0.01) break;
            terminal.options.letterSpacing = next;
          }

          // The shared window is 24 rows plus tmux's one status row. Stretch
          // those exact rows vertically instead of asking xterm for surplus
          // empty rows that appear as a gap along the pane's bottom edge.
          if (targetRows !== null) {
            for (let i = 0; i < 5; i += 1) {
              const proposed = fitAddon.proposeDimensions();
              if (!proposed || proposed.rows === targetRows) break;
              if (proposed.rows < targetRows) break;
              const current = terminal.options.lineHeight ?? BASE_LINE_HEIGHT;
              const next = responsiveLineHeight(current, proposed.rows, targetRows);
              if (Math.abs(next - current) < 0.001) break;
          ...
          terminal.resize(
            targetColumns,
            targetRows ?? Math.max(8, proposed?.rows ?? terminal.rows),
          );
        }
    ...
    void bridge.openLocalTerminal({
      ...
      targetRows = handle.windowRows + 1;
      setStatus("live");
