...
function checkoutSubjectForAgent(
  ...
  return agent === "executive"
    ? `subject:obsidience:knowledge:agent:${branch}`
  ...
  .superRefine((assignment, context) => {
    ...
    if (assignment.agent === "library") {
      context.addIssue({
        code: "custom",
        path: ["agent"],
        message: "Library cannot be a checkout target.",
      });
    }
...
export const knowledgeUncheckoutResultSchema = z
  ...
  .superRefine((result, context) => {
    ...
    if (value.agent === "library") issue("agent", "Library cannot be a checkout target.");
...
/** Curation can be assigned only to an acting agent. Library uses the
...
export const knowledgeCurationAssigneeSchema = knowledgeAgentIdSchema.refine(
  (agent) => agent !== "library",
  { message: "Library cannot be a curation assignee" },
);
