...
## Session lock and owner-authorized unlock

The existing Quickshell `LockController` owns one `WlSessionLock` with native
opaque lock surfaces on every output. Its native password prompt uses PAM;
the web graph is decoration and receives no credentials or unlock command.

The optional [session.unlock](/Tools/session.unlock.md) capability releases an
existing desktop lock through the native controller. Fresh installs do not grant
its paired Skill to the Executive. The local owner must explicitly choose this
behavior; ordinary voice transcripts do not identify the speaker. Preserve PAM,
fresh-login authentication, verified state and no replay of uncertain delivery.
