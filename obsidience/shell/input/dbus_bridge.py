...
class LockStateProjection:
    ...
    def _owner_changed(self, name, _old_owner, new_owner) -> None:
        if str(name) != SCREENSAVER_BUS:
            return
        if not str(new_owner):
            self._route(True)
            return
        self._route(self._authority_active())
