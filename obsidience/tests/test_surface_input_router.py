...
def test_dbus_lock_projection_is_atomic_and_routes_the_one_router(
    ...
    get_active.return_value = True
    projection._owner_changed(dbus_bridge.SCREENSAVER_BUS, "", ":1.200")
    assert lock_state.read_text(encoding="utf-8") == "1\n"
    get_active.return_value = False
    projection._owner_changed(dbus_bridge.SCREENSAVER_BUS, ":1.200", ":1.201")
    assert lock_state.read_text(encoding="utf-8") == "0\n"
    projection._owner_changed(dbus_bridge.SCREENSAVER_BUS, ":1.201", "")
    assert lock_state.read_text(encoding="utf-8") == "1\n"
    projection._owner_changed("org.example.Unrelated", "", ":1.202")
    assert bridge._send.call_args_list[-3:] == [
        ((dbus_bridge.DP4_FIFO, "lock"),),
        ((dbus_bridge.DP4_FIFO, "unlock"),),
        ((dbus_bridge.DP4_FIFO, "lock"),),
    ]
